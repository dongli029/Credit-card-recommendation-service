...
# json.load() 將json型態data轉成python型態, json.dump則相反
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://username:password@ip/database name'  # 'postgresql://資料庫用戶名:密碼@主機ip(127.0.0.1):port號(5432)/資料庫名稱'
db = SQLAlchemy(app)
...
2. 從您個人卡片與阿內卡信用卡資料庫交叉比對裡選出該通路前三優惠,通路名稱前以M做為指令,例如輸入:M高鐵。
        ...
        for x in range(len(characters)):
            insert_card_name = string.replace(characters[x], "")
# join(合表) CARD_NAME(代號n) 與 CREDIT_CARD_REWARD(代號r) 選出n.發卡銀行, n.別名 並融合成同一欄位(concat(n.發卡銀行, n.別名))
            def cards_top_discount(card_list, chose_channel):
                ...
                top_reward = my_cursor.fetchall() # my_cursor.fetchall() 取出全部資料
            ...
            # 模擬使用者丟進來的資料，可以任意更改
            cards_top_discount_list = (cards_top_discount([""], insert_card_name)) #預設card_list為空list, chose_channel為輸入的卡名
